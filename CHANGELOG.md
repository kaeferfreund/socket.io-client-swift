...
- A CONNECT packet without a `sid` on a `.three` manager now fires `.error` with the JS message about reaching a Socket.IO server in v2.x instead of connecting, JS-aligned with `onpacket` in `socket.io-client/lib/socket.ts`; `.two` managers are unaffected since the v2 protocol carries no sid. Found by porting the JS client's own scenario "should emit a connect_error event when reaching a Socket.IO server in v2.x".
