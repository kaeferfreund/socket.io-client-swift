...
- Polling disconnects now send a close-only POST after any in-flight POST has completed, rather than stranding the close behind `maxPayload` or overlapping requests. Queued unsent application packets are completed locally as cancelled. Shutdown remains isolated from replacement sessions and bounded by the existing one-second teardown deadline.
